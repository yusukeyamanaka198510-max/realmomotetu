...
export function TeamNameChangePanel({
  currentName,
  ...
  async function handleSubmit() {
    ...
    if (trimmedRep.length > 30) {
      setError("代表者氏名は30文字以内にしてください");
      return;
    }
    setBusy(true);
    const supabase = createClient();
    const { error } = await supabase.rpc("fn_team_rename_self", {
      p_team_name: trimmedName,
      p_representative_name: trimmedRep || null,
    });
    setBusy(false);
    if (error) {
      ...
      {open && (
        <div className="mt-2 space-y-2">
          ...
          <div className="space-y-1">
            <label className="text-xs text-zinc-500 dark:text-zinc-400">チーム名</label>
            <input
              type="text"
              placeholder="チーム名"
              value={name}
          ...
          <div className="space-y-1">
            <label className="text-xs text-zinc-500 dark:text-zinc-400">代表者氏名</label>
            <input
              type="text"
              placeholder="代表者氏名"
