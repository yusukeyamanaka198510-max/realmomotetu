/**
 * u11確認用テストデータのクリーンアップ。
 * 実行: npx tsx scripts/dev-reset-after-leaderboard-ui-test.ts
 */
...
async function main() {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    await admin.from("team_state").update({ state: "WAITING", coin_balance_cache: 0, is_paused: false }).eq("team_id", t!.id);
  }
