/**
 * u10確認用テストデータのクリーンアップ。
 * 実行: npx tsx scripts/dev-reset-after-card-ui-test.ts
 */
...
async function main() {
  ...
  await admin.from("team_state").update({
    state: "WAITING", current_turn_id: null, coin_balance_cache: 0, is_paused: false,
  }).eq("team_id", team1!.id);
  await admin.from("team_cards").delete().eq("team_id", team1!.id);
  await admin.from("card_active_effects").delete().eq("team_id", team1!.id);
  await admin.from("card_usage_log").delete().eq("team_id", team1!.id);
