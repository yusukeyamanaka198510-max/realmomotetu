-- 【本番中の緊急対応】0143でSWAP_LOCATION(場所がえカード)を修正した直後、同じ症状
-- (現在地とミッション審査中の駅がズレる)がオナラカード(PUSH_BACK_SAME_STATION_TEAMS)
-- でも発生した。原因は全く同じで、同じ駅にいる他チームのcurrent_station_idを、そのチームの
-- 状態(ミッション審査待ちかどうか)を確認せずに無条件で書き換えていたため。
-- 同じ駅にいてもDICE_READY中のチームのみ押し戻す対象にする。
--
-- あわせて、既にズレてしまったチームをその場で復旧できる本部向け関数
-- fn_admin_reassign_mission_to_current_station を追加する。対象チームの直近の
-- ミッション(AWAITING_PHOTO状態のもの)を、現在駅向けのEASYミッションに差し替える。
...
begin
  ...
  -- 足止めカード(BLOCK_UNTIL_MISSION_SUCCESS)はfn_roll_dice(通常のサイコロ)側にしか
  -- チェックが入っておらず、移動系カード(急行・特急・3進む等)を使えば足止めを無視して
  -- 移動できてしまっていた。移動を伴う効果はすべて同じ制限がかかるようにする。
  if v_card.effect_type in (
    'MOVEMENT_DICE', 'MOVEMENT_CHOSEN_DIE', 'MOVEMENT_FIXED', 'MOVEMENT_UPTO',
    'MOVEMENT_RANDOM_JUMP', 'MOVEMENT_DIRECT_TO_DEST', 'MOVEMENT_TELEPORT_TO_TEAM',
    'MOVEMENT_TO_OWNED_PROPERTY'
  ) and exists (
    select 1 from card_active_effects
  ...
  when 'BOMBII_TRANSFER' then
    ...
    perform fn_notify_team(v_event_id, p_target_team_id, format('⚔️😈%sで、ボンビーをなすりつけられました!', v_card.name));
  ...
  when 'SWAP_LOCATION' then
    if p_target_team_id is null then raise exception 'target_team_id is required'; end if;
    -- カードのマスタ説明上は「DICE_READY時」専用だが、実装にその制約が一切無く、
    -- ミッション審査待ち・到着報告待ち等、他の処理が現在駅を前提に進行中のチームの
    -- current_station_idを無条件に書き換えてしまっていた(審査中のミッションの駅と
    -- 現在地表示がズレる不具合の原因)。両チームともDICE_READY中のみ使用可能にする。
    if v_state <> 'DICE_READY' then raise exception 'invalid state: %', v_state; end if;
    if v_target_state <> 'DICE_READY' then
      raise exception 'blocked: target team is not in DICE_READY state (場所がえカード)';
    end if;
    declare v_my_station uuid := v_current_station; begin
      ...
      perform fn_notify_team(v_event_id, p_target_team_id, format('⚔️%sで場所を入れ替えられました。', v_card.name));
    end;
  ...
  when 'FORCE_NEXT_MOVE_FIXED_1' then
    ...
    if v_barrier_used then
      ...
      perform fn_notify_team(v_event_id, v_team_id, format('%sはカードバリアで防がれました。', v_card.name));
    else
      ...
      perform fn_notify_team(v_event_id, p_target_team_id, format('⚔️%sで次回移動が1マスに固定されました。', v_card.name));
    end if;
  ...
  when 'BLOCK_NEXT_MOVEMENT_CARD' then
    ...
    if v_barrier_used then
      ...
      v_message := format('%sを使用しました。', v_card.name);
      perform fn_notify_team(v_event_id, p_target_team_id, format('⚔️%sで移動カードが封印されました。', v_card.name));
    end if;
  ...
  when 'BLOCK_UNTIL_MISSION_SUCCESS' then
    ...
    if v_barrier_used then
      ...
      v_message := format('%sを使用しました。', v_card.name);
      perform fn_notify_team(v_event_id, p_target_team_id, format('⚔️%sで、ミッション成功するまで移動できなくなりました。', v_card.name));
    end if;

  when 'PUSH_BACK_SAME_STATION_TEAMS' then
    declare v_other record; v_dest uuid; v_dist int; begin
      -- SWAP_LOCATIONと同じ不具合が本番中に発生した(ミッション審査待ち等、現在駅を前提に
      -- 進行中の処理があるチームのcurrent_station_idを無条件に書き換え、審査中のミッションの
      -- 駅と現在地表示がズレていた)。同じ駅にいてもDICE_READY中のチームのみ押し戻す。
      for v_other in select id, current_station_id from team_state where event_id = v_event_id and current_station_id = v_current_station and team_id <> v_team_id and state = 'DICE_READY' loop
        ...
        if v_dest is not null then
          ...
          perform fn_notify_team(v_event_id, v_other.id, format('⚔️%sで、別の駅へ押し戻されました。', v_card.name));
        end if;
      end loop;
      v_message := format('%sを使用しました。', v_card.name);
    end;
  ...
  when 'BLOCK_NEXT_CARD_USE' then
    ...
    if v_barrier_used then
      ...
      v_message := format('%sを使用しました。', v_card.name);
      perform fn_notify_team(v_event_id, p_target_team_id, format('⚔️%sでカード使用が封印されました。', v_card.name));
    end if;
    ...
    if v_barrier_used then
      ...
      declare v_stolen record; begin
        ...
        if v_stolen.card_id is not null then
          ...
          perform fn_notify_team(v_event_id, p_target_team_id, format('⚔️%sで「%s」を奪われました。', v_card.name, v_stolen.name));
        else
    ...
    if v_barrier_used then
      ...
      declare v_destroyed record; begin
        ...
        if v_destroyed.card_id is not null then
          ...
          perform fn_notify_team(v_event_id, p_target_team_id, format('⚔️豪速球カードで「%s」を破壊されました。', v_destroyed.name));
        else
  ...
  when 'STEAL_COIN_PERCENT' then
    ...
    if v_barrier_used then
      ...
      declare v_target_balance bigint; v_amount bigint; v_pct int := coalesce((v_card.effect_value->>'percent')::int, 20); begin
        ...
        perform fn_notify_team(v_event_id, p_target_team_id, format('⚔️強奪カードで%s円奪われました。', v_amount));
      end;
    end if;
  ...
  when 'PROPERTY_TAKEOVER' then
    declare v_purchase_id uuid := (p_payload->>'purchase_id')::uuid; v_purchase record; v_balance bigint; begin
      ...
      if v_barrier_used then
        ...
        perform fn_notify_team(v_event_id, p_target_team_id, '⚔️乗っ取りカードで所有物件を奪われました。');
      end if;
    end;
...
-- ===================== fn_admin_reassign_mission_to_current_station =====================
-- SWAP_LOCATION/PUSH_BACK_SAME_STATION_TEAMSの不具合(今回の修正で解消)により、既に
-- ズレてしまったチームをその場で復旧するための本部専用関数。
-- 対象チームの直近のミッション(まだ証拠写真未提出=AWAITING_PHOTO状態のもの)を、
-- 現在駅向けの新しいEASYミッションに差し替える。既に写真提出済み(PENDING_REVIEW)の
-- 場合は先に却下してAWAITING_PHOTOへ戻してから実行すること。

create or replace function fn_admin_reassign_mission_to_current_station(p_team_id uuid, p_reason text default null)
returns void
language plpgsql security definer set search_path = public as $$
declare
...
begin
  ...
  select event_id, current_station_id, state into v_event_id, v_current_station, v_state
  ...
  if v_state <> 'MISSION_ACTIVE' then
    raise exception 'team is not in MISSION_ACTIVE state (state: %)', v_state;
  end if;

  select * into v_attempt from team_mission_attempts
    where team_id = p_team_id order by created_at desc limit 1 for update;

  if v_attempt.id is null then
    raise exception 'no mission attempt found';
  end if;
  if v_attempt.status <> 'AWAITING_PHOTO' then
    raise exception 'mission attempt is not awaiting photo (status: %). 先に却下して差し戻してください。', v_attempt.status;
  end if;
  if v_attempt.station_id = v_current_station then
    raise exception 'mission already matches current station';
  end if;

  v_new_mission := fn_pick_easy_mission(p_team_id, v_current_station);

  update team_mission_attempts
    set station_id = v_current_station,
        offered_mission_ids = array[v_new_mission],
        selected_mission_id = v_new_mission,
        locked_at = now()
    where id = v_attempt.id;

  insert into audit_log (event_id, staff_id, team_id, action_type, before_value, after_value, reason)
    values (v_staff_event_id, auth.uid(), p_team_id, 'MISSION_STATION_CORRECTED',
      jsonb_build_object('mission_attempt_id', v_attempt.id, 'old_station_id', v_attempt.station_id),
      jsonb_build_object('new_station_id', v_current_station, 'new_mission_id', v_new_mission),
      p_reason);
end;
$$;

grant execute on function fn_admin_reassign_mission_to_current_station(uuid, text) to authenticated;
