-- 「ラッキーチャンス」→「ラッキーボーナス」に名称変更し、演出用の文言を大きく表示できるよう
-- メッセージ本文に完成した案内文をそのまま埋め込む形に変更する。

alter type coin_transaction_type rename value 'LUCKY_CHANCE' to 'LUCKY_BONUS';
...
begin
  insert into coin_ledger (event_id, team_id, amount, transaction_type, idempotency_key, reason, created_by)
    values (p_event_id, p_team_id, p_amount, 'LUCKY_BONUS', gen_random_uuid(), p_reason, null);
...
-- ②: 到着マイルストーンの案内文を差し替え(ロジック本体は完全に同一)。
...
-- ②: カード使用マイルストーンの案内文を差し替え(ロジック本体は完全に同一)。
