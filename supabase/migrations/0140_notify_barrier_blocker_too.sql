-- カードバリアで妨害カードを防いだ際、防いだ側(対象チーム)にはfn_notify_team経由で
-- 通知が届いていたが、防がれた側(使用したチーム)にはRPCの戻り値(v_message)としてしか
-- 情報がなく、CardPanel.tsx側の小さな「発動!」演出+緑の小文字メッセージでしか
-- 表示されていなかった。ミッション失敗と同程度に見落とせない重要な結果なので、
-- 両チームともnotifications経由の専用モーダル(CardBarrierOverlay)で表示できるよう、
-- 防がれた側にも自チーム宛のfn_notify_teamを追加する(8箇所すべてのBLOCKED_BY_BARRIER分岐)。
