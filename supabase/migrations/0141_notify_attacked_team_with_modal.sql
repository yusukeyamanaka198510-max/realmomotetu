-- 他チームを対象とする妨害系カード(なすりつけ・場所がえ・牛歩・サイコロ封印・足止め・
-- オナラ・冬眠・刀狩り・豪速球・強奪・乗っ取り)が使われた際、対象チームへの通知は
-- 汎用の通知欄(AnnouncementBox)に小さく流れるだけで見落とされやすかった。カードバリアの
-- 防御結果と同様、専用モーダルで大きく表示できるよう、対象チーム宛の通知メッセージ先頭に
-- 目印(⚔️)を付け、クライアント側(CardAttackOverlay)でこの目印を検知してモーダル表示する。
--
-- ついでに、豪速球カード(DESTROY_RANDOM_CARD)で対象が破壊できるカードを持っていなかった
-- 場合にも、if/elseの外に置かれていたfn_notify_team呼び出しが無条件に実行され、対象へ
-- 「豪速球カードで「」を破壊されました。」という空欄入りの誤った通知が飛んでいたバグも
-- 合わせて修正する(成功時のみ通知するよう、if文の内側に移動)。
