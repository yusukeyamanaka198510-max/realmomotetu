-- カード機能: fn_use_card ディスパッチャ本体(40種のカード効果を実装)

create or replace function fn_try_consume_barrier(p_team_id uuid)
returns boolean
language plpgsql security definer set search_path = public as $$
declare
  v_barrier_card_id uuid;
  v_qty int;
begin
  select id into v_barrier_card_id from cards where card_code = 'CARD_BARRIER';
  if v_barrier_card_id is null then
    return false;
  end if;
  select quantity into v_qty from team_cards where team_id = p_team_id and card_id = v_barrier_card_id for update;
  if coalesce(v_qty, 0) > 0 then
    update team_cards set quantity = quantity - 1 where team_id = p_team_id and card_id = v_barrier_card_id;
    return true;
  end if;
  return false;
end;
...
create or replace function fn_notify_team(p_event_id uuid, p_team_id uuid, p_message text, p_usage_id uuid default null)
returns void
language plpgsql security definer set search_path = public as $$
begin
  insert into card_notifications (event_id, team_id, message, related_usage_id)
    values (p_event_id, p_team_id, p_message, p_usage_id);
end;
$$;
...
begin
  ...
  when 'GRANT_BONUS_MISSION' then
    declare v_attempt record; v_bonus_mission uuid; v_reward bigint; begin
      if v_state not in ('MISSION_SELECTION', 'MISSION_ACTIVE') then raise exception 'invalid state: %', v_state; end if;
      ...
      select id, coalesce(success_reward, case difficulty when 'EASY' then 10000000 when 'NORMAL' then 20000000 when 'HARD' then 30000000 end)
        into v_bonus_mission, v_reward
        from station_missions
        where station_id = v_attempt.station_id and is_active and id <> all (v_attempt.offered_mission_ids)
        order by random() limit 1;
      if v_bonus_mission is null then
        select id, coalesce(success_reward, 10000000) into v_bonus_mission, v_reward
          from station_missions where station_id = v_attempt.station_id and is_active order by random() limit 1;
      end if;
      if v_bonus_mission is null then raise exception 'no mission available for bonus'; end if;
      insert into team_bonus_mission_attempts (team_id, turn_id, station_id, mission_id, reward)
        values (v_team_id, v_attempt.turn_id, v_attempt.station_id, v_bonus_mission, v_reward);
      v_detail := jsonb_build_object('bonus_mission_id', v_bonus_mission, 'reward', v_reward);
      v_message := format('%sを使用しました。ボーナスミッションが追加されました。', v_card.name);
    end;
...
grant execute on function fn_try_consume_barrier(uuid) to authenticated;
