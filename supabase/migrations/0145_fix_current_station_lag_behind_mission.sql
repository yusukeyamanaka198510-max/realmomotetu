-- 【根本原因の再調査】0143/0144でSWAP_LOCATIONカード・オナラカードによる
-- current_station_idの不正な書き換えを修正したが、それとは別に、カードが一切絡まなくても
-- 「到着報告のたびに毎回」発生する、より根本的な原因が見つかった。
--
-- fn_review_arrivalの通常到着ルート(同一駅リピート到着のボーナスルートを除く)は、
-- ミッションをMISSION_ACTIVEとして即座に割り当てる一方で、team_state.current_station_id
-- 自体はここでは更新していなかった。current_station_idが実際に更新されるのは、
-- ミッションが成功と判定された時点(fn_review_mission)のみだった。
--
-- そのため、到着報告が承認されてから、そのミッションに成功と判定されるまでの間ずっと、
-- 「現在地」表示は1サイクル前の古い駅のまま表示され続け、今まさに挑戦中のミッションの
-- 駅と食い違って見えていた(例: 現在地は清澄白河のままなのに、挑戦中のミッションは
-- 到着済みの船堀のもの)。これはカードの有無に関わらず、毎回の到着で必ず起きていた。
--
-- 到着報告が承認された時点でcurrent_station_idも新しい駅に更新するよう修正する。
...
begin
  ...
  -- 到着報告が承認された時点でcurrent_station_idも新しい駅に更新する。
  -- (旧実装はミッション成功が確定するfn_review_missionまでcurrent_station_idを
  --  更新しておらず、「現在地」表示がミッション判定待ちの間ずっと1サイクル前の
  --  古い駅のまま表示され続け、今まさに挑戦中のミッションの駅とズレて見えていた。
  --  この間に他チームのカード効果が及ぶこと自体は0143/0144で別途防止済みだが、
  --  カードが一切絡まなくても到着のたびに必ずこのズレが発生していたため、
  --  ここで根本的に修正する。)
  ...
  update team_state set current_station_id = v_station_id, state = 'MISSION_ACTIVE', version = version + 1 where team_id = v_team_id;
  update review_queue set status = 'RESOLVED'
    where event_id = v_event_id and team_id = v_team_id and type = 'ARRIVAL' and ref_id = p_arrival_submission_id;
