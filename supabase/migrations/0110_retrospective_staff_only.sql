-- fn_retrospective_timeline を本部限定に変更する。
-- 参加者の顔写真・却下理由・ボンビー付与記録などを含むため、参加者には見せない方針に変更した
-- (/retrospective ページ自体も本部専用に変更済み)。
...
begin
  ...
  with combined as (
    ...
    select ma.team_id, ma.reviewed_at, 'MISSION',
      jsonb_build_object(
        'title', sm.title, 'difficulty', sm.difficulty, 'result', ma.status,
    ...
    select ba.team_id, ba.reviewed_at, 'BONUS_MISSION',
      jsonb_build_object('title', ba.title, 'result', ba.status, 'reward', ba.reward),
