-- fn_admin_rehearsal_resetが実際に失敗するバグを修正。
-- coin_ledger.related_mission_attempt_id/related_turn_idにはon delete cascadeが
-- 付いていないため、team_mission_attempts/turnsを先に消すと外部キー制約違反になる。
-- coin_ledgerを一番先に削除する順序に直す。
