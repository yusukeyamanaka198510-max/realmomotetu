-- ルール追加: カードは1ターンにつき1枚まで(種類を問わない)。
-- 「ターン」はteam_state.current_turn_idで判定する。移動系カードを使うとその場で新しい
-- turnが作られる(fn_finalize_movement)ため、移動カード使用後に別のカードを使おうとすると
-- 同じ新turnに対して既に1件ログがある状態になり、正しくブロックされる。
-- 一度も移動したことがない(current_turn_idがnull)チームの最初の1枚は許可する。
