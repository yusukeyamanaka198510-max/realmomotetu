-- 足止めカード(STOPPED / BLOCK_UNTIL_MISSION_SUCCESS)が、選択したチームで
-- 確定して使用しても「null value in column "remaining_uses" of relation
-- "card_active_effects" violates not-null constraint」というDBエラーで
-- 常に失敗していたことが判明した(card_active_effects.remaining_usesは
-- not null default 1のカラムだが、このcase分岐だけnullを渡していた)。
--
-- この効果の有効/解除判定はどこも consumed_at is null だけを見ており、
-- remaining_uses は実質使われていないため、単純に1を渡すよう修正する
-- (0038以前の実装と同じ値に戻す形)。
