-- 場所がえカード(SWAP_LOCATION)は、カードマスタの説明上「DICE_READY時」専用のはずだが、
-- 実装にその制約が一切無かった。他の移動系カードは使用可能な状態(DICE_READY)かを
-- チェックしているのに対し、SWAP_LOCATIONは使用する側・対象側どちらの状態も確認せずに
-- team_state.current_station_idを無条件に書き換えていた。
--
-- このため、対象チームがミッション審査待ち(MISSION_ACTIVE/MISSION_REVIEW)や到着報告待ち
-- (ARRIVAL_SUBMISSION/ARRIVAL_REVIEW)等で「現在駅」を前提に進行中の処理があるタイミングで
-- 場所がえカードを使われると、その処理(審査中のミッション等)は元の駅のまま残るのに
-- current_station_idだけが書き換わり、「現在地」表示と審査中ミッションの駅がズレる
-- 不具合が発生していた(例: 現在地が「清澄白河」なのに、審査中のミッションは「船堀」)。
--
-- 使用する側・対象側の両方がDICE_READY状態の時のみ使用可能にする。
