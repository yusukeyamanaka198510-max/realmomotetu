-- 足止めカード(BLOCK_UNTIL_MISSION_SUCCESS)を使われたチームが、通常のサイコロ(fn_roll_dice)は
-- 拒否されるのに、急行・特急・3進むカードなどの移動系カードを使えば制限を無視してそのまま
-- 移動できてしまっていた不具合の修正。
--
-- fn_roll_diceにはBLOCK_UNTIL_MISSION_SUCCESSのチェックがあったが、fn_use_card側の
-- 移動系effect_type(MOVEMENT_DICE/MOVEMENT_FIXED/MOVEMENT_CHOSEN_DIE/MOVEMENT_UPTO/
-- MOVEMENT_RANDOM_JUMP/MOVEMENT_DIRECT_TO_DEST/MOVEMENT_TELEPORT_TO_TEAM/
-- MOVEMENT_TO_OWNED_PROPERTY)にはこのチェックが一つも入っていなかった。
