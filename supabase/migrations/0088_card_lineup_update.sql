-- カードラインナップの見直し:
-- - 特急カードをサイコロ4個、新幹線カードをサイコロ5個に変更
-- - のぞみカード(サイコロ5個)は新幹線カードと重複するため削除
-- - 徳政令カードは本ゲームにマイナスコインの概念が無く常に無効(enabled=false)なため削除
-- - はやぶさカード(サイコロ6個)、急行周遊カード(3回使える急行カード)、
--   スペシャルカード(出目を1〜6から自分で選べる)を新規追加

update cards set effect_value = '{"dice_count":4}'::jsonb where card_code = 'LIMITED_EXPRESS';
update cards set effect_value = '{"dice_count":5}'::jsonb where card_code = 'SHINKANSEN';

-- 削除対象カードへの参照を先に片付ける(FKにon delete cascadeが無いテーブルがあるため)
delete from team_cards where card_id in (select id from cards where card_code in ('NOZOMI', 'DEBT_FORGIVE'));
delete from card_usage_log where card_id in (select id from cards where card_code in ('NOZOMI', 'DEBT_FORGIVE'));
update card_active_effects set source_card_id = null where source_card_id in (select id from cards where card_code in ('NOZOMI', 'DEBT_FORGIVE'));
delete from station_card_pool where card_id in (select id from cards where card_code in ('NOZOMI', 'DEBT_FORGIVE'));
delete from cards where card_code in ('NOZOMI', 'DEBT_FORGIVE');

insert into cards (card_code, name, category, rarity, description, effect_type, effect_value, use_timing, target_type, exchangeable, enabled) values
('HAYABUSA', 'はやぶさカード', 'MOVEMENT', 'SUPER_RARE', 'サイコロを6個振り、合計値を移動可能数とする。', 'MOVEMENT_DICE', '{"dice_count":6}', 'DICE_READY時', 'NONE', true, true),
('EXPRESS_LOOP', '急行周遊カード', 'MOVEMENT', 'RARE', '使用後3回の移動について、通常サイコロを2個振れる(移動カード使用時は消費しない)。', 'HOT_STREAK', '{"dice_count":2,"uses":3}', 'いつでも', 'SELF', true, true),
('DICE_CHOICE', 'スペシャルカード', 'MOVEMENT', 'SUPER_RARE', 'サイコロの出目を1〜6から自分で選び、その数だけ移動する。', 'MOVEMENT_CHOSEN_DIE', '{}', 'DICE_READY時', 'NONE', true, true);

-- fn_use_card に MOVEMENT_CHOSEN_DIE(スペシャルカード)の分岐だけを追加する。
-- それ以外は0071_bombii_system.sql時点の最新定義から一切変更していない(逐語コピー)。
...
begin
  ...
  -- 以下の2件は0022時点では実装されていたが、その後の全文置き換え(0027/0028/0038/0071)で
  -- 誤って欠落し、使用すると必ず'unimplemented effect_type'で失敗する状態になっていた
  -- (目的地予想カード・おすそわけカードが実質使用不能というバグ)。今回のカード見直しに
  -- 合わせて復元する。
