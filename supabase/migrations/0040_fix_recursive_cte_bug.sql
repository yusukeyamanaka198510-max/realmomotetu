-- 重大バグ修正: fn_pick_next_destination / fn_shortest_hops / fn_stations_within が
-- 「NOT EXISTS (再帰CTE自身を参照するサブクエリ)」という、PostgreSQLで許可されていない
-- 構文を使っていたため、特定の条件下で必ず
--   ERROR: recursive reference to query "bfs" must not appear within a subquery
-- となっていた(目的地到達承認・カードの距離判定などが失敗しうる状態だった)。
-- fn_reachable_stations と同じ「訪問済み配列を引き回す」方式に統一して修正する。
