-- ラッキーチャンス機能:
-- ①1時間に1回、資産下位3チーム(最下位/下から2番/下から3番)に3000万/2000万/1000万円を配布
-- ②全チーム共通で「到着」「カード使用」それぞれの累計回数が10/20/30回に達した瞬間、
--   達成したチームへ3000万円を配布(それぞれ独立にカウント、一度きり)

alter type coin_transaction_type add value if not exists 'LUCKY_CHANCE';

alter table events add column if not exists last_lucky_hourly_run_at timestamptz;
alter table team_state add column if not exists arrival_count int not null default 0;
alter table team_state add column if not exists card_use_count int not null default 0;

-- 付与処理の共通ヘルパー(コイン加算+通知)。内部専用のためpublicへの実行権限は与えない。
create or replace function fn_grant_lucky_bonus(p_team_id uuid, p_event_id uuid, p_amount bigint, p_reason text, p_message text)
returns void
language plpgsql security definer set search_path = public as $$
begin
  insert into coin_ledger (event_id, team_id, amount, transaction_type, idempotency_key, reason, created_by)
    values (p_event_id, p_team_id, p_amount, 'LUCKY_CHANCE', gen_random_uuid(), p_reason, null);
  update team_state set coin_balance_cache = coin_balance_cache + p_amount where team_id = p_team_id;
  perform fn_notify_team(p_event_id, p_team_id, p_message);
end;
$$;
revoke execute on function fn_grant_lucky_bonus(uuid, uuid, bigint, text, text) from public;

-- ①: 1時間に1回、誰か(参加者・本部いずれか)がアクセスするたびに軽く呼ばれ、
-- 前回実行から60分過ぎていれば資産下位3チームにボーナスを配る。
...
begin
  ...
  if v_last_run is not null and now() - v_last_run < interval '60 minutes' then
    return;
  end if;
  ...
  loop
    if v_row.rnk between 1 and 3 then
      perform fn_grant_lucky_bonus(
        ...
        format('🍀ラッキーチャンス!%sボーナスで+%s円を獲得しました!', v_labels[v_row.rnk], to_char(v_amounts[v_row.rnk], 'FM999,999,999'))
      );
    end if;
  end loop;
end;
$$;
grant execute on function fn_maybe_run_lucky_hourly_bonus() to authenticated;

-- ===================== ②: fn_review_arrival(到着承認)に到着回数の累積判定を追加 =====================
...
begin
  ...
  if v_new_arrival_count in (10, 20, 30) then
    perform fn_grant_lucky_bonus(
      ...
      format('到着%s回達成ラッキーチャンス', v_new_arrival_count),
      format('🍀ラッキーチャンス!到着%s回達成で+30,000,000円を獲得しました!', v_new_arrival_count)
    );
  end if;
...
-- ===================== ②: fn_use_card(カード使用)にカード使用回数の累積判定を追加 =====================
...
begin
  ...
  if v_new_card_use_count in (10, 20, 30) then
    perform fn_grant_lucky_bonus(
      v_team_id, v_event_id, 30000000,
      format('カード使用%s回達成ラッキーチャンス', v_new_card_use_count),
      format('🍀ラッキーチャンス!カード使用%s回達成で+30,000,000円を獲得しました!', v_new_card_use_count)
    );
  end if;
