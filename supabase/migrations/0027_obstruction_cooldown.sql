-- 妨害カードのクールタイム: 同一チームから同じ対象チームへの妨害カード連続使用を防ぐ。
-- 秒数は管理画面から変更可能。

alter table events add column obstruction_cooldown_seconds int not null default 60;

create or replace function fn_admin_set_obstruction_cooldown(p_seconds int)
returns void
...
begin
  ...
  update events set obstruction_cooldown_seconds = greatest(p_seconds, 0) where id = v_staff_event_id;
end;
$$;
grant execute on function fn_admin_set_obstruction_cooldown(int) to authenticated;
