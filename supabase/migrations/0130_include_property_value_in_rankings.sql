-- ⑧ 順位表示(参加者画面・振り返り画面)と優勝判定が、コイン残高(coin_balance_cache)
-- だけで計算されており、保有不動産の価値が一切反映されていなかった。物件を買うほど
-- 順位が下がって見える不具合だった。「現金 + 保有中(未決済)の物件の購入額合計」を
-- total_assets として算出し、これで順位付けするよう修正する。
...
begin
  ...
  select jsonb_agg(jsonb_build_object(
    ...
    into v_rows
  from (
    select ts.team_id, ts.coin_balance_cache,
      ts.coin_balance_cache + coalesce(pp.total, 0) as total_assets,
      t.team_name, s.name as station_name, ts.has_bombii,
...
-- fn_retrospective_timeline: 0111の内容そのまま、ランキング算出部分だけをtotal_assets基準に変更。
