-- fn_claim_mission_rewardのボンビー悪さ処理: 未代入のrecord変数のフィールド参照はエラーになるため、
-- スカラー変数(v_purchase_id/v_purchase_price)に置き換えて修正する。
-- また price_paid 列を select していなかった不足も合わせて直す。
